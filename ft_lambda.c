...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/06 16:13:24 by htillman          #+#    #+#             */
/*   Updated: 2019/03/06 17:36:30 by htillman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_sm(t_fractol *dat)
{
	...
	p = dat->set.old_re - dat->set.old_re *
		dat->set.old_re + dat->set.old_im * dat->set.old_im;
	q = dat->set.old_im - 2 * dat->set.old_re * dat->set.old_im;
	dat->set.new_re = dat->set.c_re * p - dat->set.c_im * q;
	dat->set.new_im = dat->set.c_re * q + dat->set.c_im * p;
}

static void	ft_if_m(t_fractol *dat, t_x *l, int i)
{
	l->color = dat->set.color + dat->l_color * i;
